...
export default function CampaignsPage({ addToast }: { addToast?: (msg: string, type?: string) => void }) {
  ...
  async function handleSubmit(e?: React.FormEvent) {
    ...
    try {
      ...
      setName(""); setSubject(""); setBody(""); setTemplateId(null);
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <select value={templateId || ""} onChange={(e) => handleTemplateChange(e.target.value)} className="w-full px-4 py-3 border border-gray-200 rounded-xl">
          ...
          <div>
            <button type="submit" className="bg-gradient-to-r from-purple-600 to-purple-700 text-white px-6 py-3 rounded-xl">
              Create Campaign
            </button>
          </div>
        </form>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div className="divide-y divide-gray-100">
          {campaigns.map((c) => (
            <div key={c.id} className="p-6 hover:bg-gray-50 flex flex-col lg:flex-row justify-between gap-4 lg:gap-0 items-start lg:items-center">
              <div>
                <h3 className="text-lg font-semibold text-gray-900">{c.name}</h3>
                <p className="text-gray-600 text-sm">Subject: {c.subject}</p>
                <p className="text-gray-600 text-sm">Template: {c.template?.name || "None"}</p>
                <p className="text-gray-600 text-sm">Subscribers: {c.subscribers?.length || 0}</p>
                <p className={`text-sm font-medium ${c.status === "sent" ? "text-green-600" : "text-gray-500"}`}>Status: {c.status}</p>
              </div>

              <div className="flex gap-2">
                {c.status !== "sent" && (
                  <button onClick={() => handleSend(c.id)} className="px-4 py-2 bg-purple-600 text-white rounded-xl">
                    Send Now
                  </button>
                )}
              </div>
